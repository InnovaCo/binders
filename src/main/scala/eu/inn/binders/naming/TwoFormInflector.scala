...
private[naming] case class CategoryRule(singularList: Seq[String], singular: String, plural: String) extends Rule {
...
abstract class TwoFormInflector {
  ...
  def getPlural(word: String): String = {
    rules.toStream.flatMap(_.getPlural(word)).headOption.getOrElse(word)
  }

  def getSingular(word: String): String = {
